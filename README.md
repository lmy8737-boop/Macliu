...
**已经有自己的知识库？** 固定把这个仓库 clone 到 `~/Documents/投研方法论工具`（Windows：`%USERPROFILE%\Documents\投研方法论工具`）——不用另外想放哪，也不要 clone 进你自己的 vault 里。让你的 agent 读 `<这个路径>/SKILL.md` 加载方法论，但写研究内容时用**你自己知识库里已有的目录**（不是这个仓库里的路径）——《知识库写入规范》讲的是原位更新、双链、结构化数据分离这些原则，不绑定具体文件夹名，直接套用到你自己的分类习惯上即可。
